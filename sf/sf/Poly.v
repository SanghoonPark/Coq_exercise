...
Definition filter_even_gt7 (l : list nat) : list nat :=
  (* FILL IN HERE *)
  filter (fun x => blt_nat 7 x) (filter (fun y => evenb y) l).
...
Theorem snoc_append : forall (X: Type) (l: list X) (n: X), snoc l n = l++[n].
Proof.
  intros X l n. induction l. reflexivity.
  simpl. rewrite -> IHl. reflexivity.
Qed.

Theorem map_append : forall (X Y: Type) (f: X->Y) (l1 l2: list X),
  map f (l1++l2) = map f l1 ++ map f l2.
Proof.
  intros X Y f l1 l2.
  induction l1 as [| h t]. reflexivity.
...
Proof.
  ...
  simpl. rewrite -> snoc_append. rewrite -> snoc_append. rewrite -> map_append. rewrite -> IHt.
  reflexivity.
Qed.
(** [] *)
